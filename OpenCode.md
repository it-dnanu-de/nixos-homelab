...
**Memory layer (2026-08-08) — set up now, activated when work starts:** a **pgvector RAG memory** on the Arch dev machine (NOT the Dell). Components: local Postgres + **pgvector**, **Ollama + nomic-embed-text** for embeddings (~500MB, private, no API cost), and a **memory MCP server** (candidate: **AtomicMemory** — TS, portable, MCP-native, leading BEAM benchmarks, local-first) as the agent interface. Ingest: OpenCode.md, README, AGENTS, Changes.md, Memory.md, TODO/, git history. Agents **retrieve-on-demand** (only relevant chunks load → minimal context; replaces loading Memory.md whole). **Turned on when the human is ready to start building** — infra designed now, enabled on command. *Multimodal note (verified via MCP): most agent models accept image/file input (paste screenshots); verifier + deployer are text-only; Muse Spark 1.2 is the most capable (text+image+video+file+audio). OpenRouter has NO embedding models (verified) — local Ollama is required for the RAG.*
