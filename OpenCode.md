...
**Memory layer (2026-08-08) — set up now, activated when work starts:** a **pgvector RAG memory** (local Postgres + pgvector on the Arch dev machine, memory MCP server as the agent interface). **Embeddings via OpenRouter: `nvidia/nemotron-3-embed-1b:free`** (2048-dim, 32768 ctx, **free**, verified working on this account) — no local Ollama needed. Ingest: OpenCode.md, README, AGENTS, Changes.md, Memory.md, TODO/, git history. Agents **retrieve-on-demand** (only relevant chunks load → minimal context; replaces loading Memory.md whole). **Turned on when the human is ready to start building** — infra designed now, enabled on command. *Embeddings on OpenRouter: 33 models, listed via `output_modalities=embeddings`; `bge-m3`/`text-embedding-3-small`/`gemini-embedding-2` exist but need extra providers enabled — `nemotron-3-embed-1b:free` is the one that works on our account today. Multimodal (verified via MCP): most agent models accept image/file input; verifier + deployer are text-only; Muse Spark 1.2 is the most capable.*
